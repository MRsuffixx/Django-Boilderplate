...
def test_two_factor_setup_encrypts_secret_and_returns_single_use_recovery_codes(user):
    secret, uri = TwoFactorService.begin_setup(user=user, password="A-very-secure-test-password-42")
...
def test_two_factor_can_be_disabled_with_password_and_totp(user):
    secret, _ = TwoFactorService.begin_setup(user=user, password="A-very-secure-test-password-42")
