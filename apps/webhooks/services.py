...
class WebhookService:
    ...
    def validate_url(url: str) -> None:
        parsed = urlparse(url)
        if parsed.scheme not in ({"https"} if settings.APP_ENV == "production" else {"http", "https"}):
        ...
        for address in addresses:
            ...
            if not ip.is_global:
                raise ValidationError("Webhook URLs cannot resolve to private or reserved addresses.")

    @classmethod
    @transaction.atomic
    def create_endpoint(cls, *, owner, name: str, url: str, event_types: list[str]) -> tuple[WebhookEndpoint, str]:
    ...
    @classmethod
    def signature_headers(cls, *, endpoint: WebhookEndpoint, event: WebhookEvent, body: bytes) -> dict[str, str]:
    ...
    def serialize_event(event: WebhookEvent) -> bytes:
        return json.dumps(
            {"id": str(event.pk), "type": event.event_type, "created_at": event.created_at.isoformat(), "data": event.payload},
            separators=(",", ":"),
            sort_keys=True,
        ).encode()
