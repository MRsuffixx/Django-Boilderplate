...
class PermissionService:
    ...
    def has_permission(user, codename: str) -> bool:
        if not user or not user.is_authenticated or not user.is_active or user.status != AccountStatus.ACTIVE:
            return False
        override = (
    ...
    def permission_codes(user) -> set[str]:
        ...
        overrides = dict(
            user.permission_overrides.values_list("permission__codename", "effect")
        )
        if user.is_superuser:
            from apps.authorization.models import Permission

            role_codes = set(Permission.objects.values_list("codename", flat=True))
        role_codes.update(code for code, effect in overrides.items() if effect == OverrideEffect.ALLOW)
        role_codes.difference_update(code for code, effect in overrides.items() if effect == OverrideEffect.DENY)
        return role_codes
